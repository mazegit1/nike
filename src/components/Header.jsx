    ...
    <>
      <div className="header fixed w-full bg-white shadow-2xl flex items-center justify-between md:px-48 px-12 py-2 z-10">
        <div className="header-left">
          <Link to="/"> <SiNike className='text-4xl md:text-7xl' /> </Link>
        </div>
        ...
        <div className="header-right text-2xl md:text-4xl flex items-center gap-4 md:gap-6 font-medium">
          ...
          {searchOpen && (
            <motion.input
              ...
              animate={{ width: ['100px', '150px', '250px', '300px'], opacity: 1 }}
          ...
          <Link to="/cart">
            <div className="relative cursor-pointer">
              ...
              {cartItems.length > 0 && (
                <span className="absolute top-0 right-0 w-4 h-4 bg-red-500 text-white rounded-full text-sm flex items-center justify-center">
                  {cartItems.length}
                </span>
              )}
            </div>
